...
#define DEF_OP(x) void Arm64JITCore::Op_##x(IR::IROp_Header const* IROp, IR::NodeID Node)
  ...
  void Arm64JITCore::VFScalarOperation(
  ...
  void Arm64JITCore::VFScalarUnaryOperation(
  ...
  DEF_OP(LoadNamedVectorConstant) {
    ...
    auto GenerateMemOperand = [this](uint8_t OpSize, uint32_t NamedConstant, FEXCore::ARMEmitter::Register Base) {
      ...
      if (
      ConstantOffset <= 255 || // Unscaled 9-bit signed
  ...
  DEF_OP(VFAddV) {
    ...
    LOGMAN_THROW_AA_FMT(
    OpSize == Core::CPUState::XMM_SSE_REG_SIZE || OpSize == Core::CPUState::XMM_AVX_REG_SIZE, "Only AVX and SSE size supported");
    LOGMAN_THROW_AA_FMT(ElementSize == 2 || ElementSize == 4 || ElementSize == 8, "Invalid size");
    const auto SubRegSize = ARMEmitter::ToVectorSizePair(
    ElementSize == 2 ? ARMEmitter::SubRegSize::i16Bit
    : ElementSize == 4
