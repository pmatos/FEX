...
#define DEF_OP(x) void Arm64JITCore::Op_##x(IR::IROp_Header const *IROp, IR::NodeID Node)
  ...
  DEF_OP(LoadRegister) {
    ...
    if (Op->Class == IR::GPRClass) {
      const auto regId = Op->Offset == offsetof(Core::CpuStateFrame, State.pf_raw) ? (StaticRegisters.size() - 2)
      ...
      case 4:
        ...
        if (GetReg(Node).Idx() != reg.Idx()) mov(GetReg(Node).W(), reg.W());
        break;
  ...
  DEF_OP(LoadFlag) {
    ...
    LOGMAN_THROW_A_FMT(
    Op->Flag != X86State::RFLAG_PF_RAW_LOC && Op->Flag != X86State::RFLAG_AF_RAW_LOC, "PF/AF must be accessed as registers");
  ...
  FEXCore::ARMEmitter::SVEMemOperand Arm64JITCore::GenerateSVEMemOperand(
  uint8_t AccessSize, FEXCore::ARMEmitter::Register Base, IR::OrderedNodeWrapper Offset, IR::MemOffsetType OffsetType,
  ...
  DEF_OP(VLoadVectorElement) {
    ...
    LOGMAN_THROW_AA_FMT(
    ElementSize == 1 || ElementSize == 2 || ElementSize == 4 || ElementSize == 8 || ElementSize == 16, "Invalid element size");
