...
#include "Interface/Core/VSyscall/VSyscall.inc"
  ...
  bool Decoder::NormalOp(FEXCore::X86Tables::X86InstInfo const *Info, uint16_t Op, DecodedHeader Options) {
    ...
    LOGMAN_THROW_AA_FMT(
    !(Info->Type >= FEXCore::X86Tables::TYPE_GROUP_1 && Info->Type <= FEXCore::X86Tables::TYPE_GROUP_P),
    ...
    bool HasXMMSrc = HasXMMFlags && !HAS_XMM_SUBFLAG(Info->Flags, InstFlags::FLAGS_SF_SRC_GPR) &&
    !HAS_XMM_SUBFLAG(Info->Flags, InstFlags::FLAGS_SF_MMX_SRC);
    bool HasXMMDst = HasXMMFlags && !HAS_XMM_SUBFLAG(Info->Flags, InstFlags::FLAGS_SF_DST_GPR) &&
    !HAS_XMM_SUBFLAG(Info->Flags, InstFlags::FLAGS_SF_MMX_DST);
    bool HasMMSrc = HasXMMFlags && !HAS_XMM_SUBFLAG(Info->Flags, InstFlags::FLAGS_SF_SRC_GPR) &&
    HAS_XMM_SUBFLAG(Info->Flags, InstFlags::FLAGS_SF_MMX_SRC);
    bool HasMMDst = HasXMMFlags && !HAS_XMM_SUBFLAG(Info->Flags, InstFlags::FLAGS_SF_DST_GPR) &&
    HAS_XMM_SUBFLAG(Info->Flags, InstFlags::FLAGS_SF_MMX_DST);
    ...
    // New instruction size decoding
    {
      ...
      (HasWideningDisplacement || DstSizeFlag == FEXCore::X86Tables::InstFlags::SIZE_64BIT || DstSizeFlag == FEXCore::X86Tables::InstFlags::SIZE_64BITDEF)) {
      ...
      (HasWideningDisplacement || SrcSizeFlag == FEXCore::X86Tables::InstFlags::SIZE_64BIT || SrcSizeFlag == FEXCore::X86Tables::InstFlags::SIZE_64BITDEF)) {
    ...
    if (
    HAS_NON_XMM_SUBFLAG(Info->Flags, FEXCore::X86Tables::InstFlags::FLAGS_SF_DST_RAX) ||
    HAS_NON_XMM_SUBFLAG(Info->Flags, FEXCore::X86Tables::InstFlags::FLAGS_SF_DST_RDX)) {
      ...
      CurrentDest->Data.GPR.GPR = HAS_NON_XMM_SUBFLAG(Info->Flags, FEXCore::X86Tables::InstFlags::FLAGS_SF_DST_RAX)
      ...
      if (CurrentDest->Data.GPR.GPR == FEXCore::X86State::REG_INVALID) return false;
    }
    ...
    auto ModRMOperand =
    [&](
    FEXCore::X86Tables::DecodedOperand &GPR, FEXCore::X86Tables::DecodedOperand &NonGPR, bool HasXMMGPR, bool HasXMMNonGPR, bool HasMMGPR,
    bool HasMMNonGPR, bool GPR8Bit, bool NonGPR8Bit) {
      ...
      if (GPR.Data.GPR.GPR == FEXCore::X86State::REG_INVALID) return false;
      ...
      if (ModRM.mod == 0b11) {
        ...
        if (NonGPR.Data.GPR.GPR == FEXCore::X86State::REG_INVALID) return false;
      } else {
    ...
    if (Bytes != 0) {
      ...
      if (
      (Info->Flags & FEXCore::X86Tables::InstFlags::FLAGS_SRC_SEXT) ||
...
#undef OPD
  ...
  bool Decoder::DecodeInstruction(uint64_t PC) {
    ...
    for (;;) {
      if (InstructionSize >= MAX_INST_SIZE) return false;
      uint8_t Op = ReadByte();
      switch (Op) {
      case 0x0F: { // Escape Op
        ...
        case 0x3A: { // F3A Table!
          ...
          if (DecodeInst->Flags & DecodeFlags::FLAG_REX_WIDENING) Prefix |= PF_3A_REX;
      ...
      default:
        [[likely]] { // Default base table
          ...
          if (Info->Type == FEXCore::X86Tables::TYPE_REX_PREFIX) {
            ...
            // XGPR_B bit set
            if (Op & 0b0001) DecodeInst->Flags |= DecodeFlags::FLAG_REX_XGPR_B;

            // XGPR_X bit set
            if (Op & 0b0010) DecodeInst->Flags |= DecodeFlags::FLAG_REX_XGPR_X;

            // XGPR_R bit set
            if (Op & 0b0100) DecodeInst->Flags |= DecodeFlags::FLAG_REX_XGPR_R;
          } else {
  ...
  void Decoder::BranchTargetInMultiblockRange() {
    if (!CTX->Config.Multiblock) return;
  ...
  uint8_t const *_InstStream, uint64_t PC, uint64_t MaxInst,
