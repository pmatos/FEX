...
namespace FEXCore::X86Tables {
  ...
  struct DecodedOperand {
    ...
    bool IsNone() const { return Type == OpType::Nothing; }
    bool IsGPR() const { return Type == OpType::GPR; }
    bool IsGPRDirect() const { return Type == OpType::GPRDirect; }
    bool IsGPRIndirect() const { return Type == OpType::GPRIndirect; }
    bool IsRIPRelative() const { return Type == OpType::RIPRelative; }
    bool IsLiteral() const { return Type == OpType::Literal; }
    bool IsSIB() const { return Type == OpType::SIB; }
...
#define HAS_NON_XMM_SUBFLAG(x, flag) (((x) & (FEXCore::X86Tables::InstFlags::FLAGS_XMM_FLAGS | (flag))) == (flag))
  ...
  struct X86InstInfo {
    ...
    bool operator==(const X86InstInfo &b) const {
      if (strcmp(Name, b.Name) != 0 || Type != b.Type || Flags != b.Flags || MoreBytes != b.MoreBytes) return false;

      // We don't care if the opcode dispatcher differs
      return true;
    }
    bool operator!=(const X86InstInfo &b) const { return !operator==(b); }
  };
  ...
  template<typename OpcodeType> struct X86TablesInfoStruct {
    OpcodeType first;
    uint8_t second;
    X86InstInfo Info;
  };
  ...
  template<typename OpcodeType> constexpr static inline void
  GenerateTableWithCopy(X86InstInfo *FinalTable, X86TablesInfoStruct<OpcodeType> const *LocalTable, size_t TableSize, X86InstInfo *OtherLocal) {
    for (size_t j = 0; j < TableSize; ++j) {
      X86TablesInfoStruct<OpcodeType> const &Op = LocalTable[j];
      auto OpNum = Op.first;
      X86InstInfo const &Info = Op.Info;
...
template<> struct fmt::formatter<FEXCore::X86Tables::DecodedOperand::OpType> : formatter<uint32_t> {
  template<typename FormatContext> auto format(FEXCore::X86Tables::DecodedOperand::OpType type, FormatContext &ctx) const {
