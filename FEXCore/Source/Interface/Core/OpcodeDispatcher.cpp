...
#define OpcodeArgs [[maybe_unused]] FEXCore::X86Tables::DecodedOp Op
  ...
  template<bool IsSyscallInst> void OpDispatchBuilder::SyscallOp(OpcodeArgs) {
    ...
    if (
    OSABI != FEXCore::HLE::SyscallOSABI::OS_HANGOVER &&
...
#define OPD(group, prefix, Reg) (((group - FEXCore::X86Tables::TYPE_GROUP_1) << 6) | (prefix) << 3 | (Reg))
    ...
    case OPD(FEXCore::X86Tables::TYPE_GROUP_1, OpToIndex(0x83), 4): IROp = FEXCore::IR::IROps::OP_AND; break;
...
#undef OPD
    ...
    bool AllowUpperGarbage = IROp == FEXCore::IR::IROps::OP_AND || IROp == FEXCore::IR::IROps::OP_XOR || IROp == FEXCore::IR::IROps::OP_OR;
  ...
  template<uint32_t SrcIndex> void OpDispatchBuilder::ADCOp(OpcodeArgs) {
    // Calculate flags early.
    CalculateDeferredFlags();
    ...
    if (Size < 4) Result = _Bfe(IR::SizeToOpSize(std::max<uint8_t>(4u, Size)), Size * 8, 0, Result);
    GenerateFlags_ADC(Op, Result, Before, Src, CF);
  }

  template<uint32_t SrcIndex, bool SetFlags> void OpDispatchBuilder::SBBOp(OpcodeArgs) {
    // Calculate flags early.
    CalculateDeferredFlags();
  ...
  template<uint32_t SegmentReg> void OpDispatchBuilder::PUSHSegmentOp(OpcodeArgs) {
  ...
  template<uint32_t SegmentReg> void OpDispatchBuilder::POPSegmentOp(OpcodeArgs) {
    const uint8_t SrcSize = GetSrcSize(Op);
    const uint8_t DstSize = GetDstSize(Op);
  ...
  OrderedNode *OpDispatchBuilder::SelectBit(OrderedNode *Cmp, bool TrueIsNonzero, IR::OpSize ResultSize, OrderedNode *TrueValue, OrderedNode *FalseValue) {
    ...
    if (IsValueConstant(WrapNode(TrueValue), &TrueConst) && IsValueConstant(WrapNode(FalseValue), &FalseConst) && TrueConst == 1 && FalseConst == 0) {
      ...
      if (!TrueIsNonzero) Cmp = _Not(OpSize::i32Bit, Cmp);
  ...
  OrderedNode *OpDispatchBuilder::SelectCC(uint8_t OP, IR::OpSize ResultSize, OrderedNode *TrueValue, OrderedNode *FalseValue) {
    ...
    if (!Complex) return _NZCVSelect(ResultSize, Cond, TrueValue, FalseValue);
  ...
  void OpDispatchBuilder::LoopOp(OpcodeArgs) {
    ...
    if (CheckZF) AndCondWith = GetRFLAG(FEXCore::X86State::RFLAG_ZF_RAW_LOC, !ZFTrue);
    ...
    if (AndCondWith) SrcCond = _And(OpSize::i64Bit, SrcCond, AndCondWith);
  ...
  template<uint32_t SrcIndex> void OpDispatchBuilder::TESTOp(OpcodeArgs) {
  ...
  template<uint32_t SrcIndex> void OpDispatchBuilder::CMPOp(OpcodeArgs) {
  ...
  void OpDispatchBuilder::XCHGOp(OpcodeArgs) {
    ...
    if (
    Op->OP == 0x90 && GetSrcSize(Op) >= 4 && Op->Src[0].IsGPR() && Op->Src[0].Data.GPR.GPR == FEXCore::X86State::REG_RAX &&
  ...
  template<bool ToSeg> void OpDispatchBuilder::MOVSegOp(OpcodeArgs) {
  ...
  template<bool SHL1Bit> void OpDispatchBuilder::SHLOp(OpcodeArgs) {
    OrderedNode *Src{};
  ...
  template<bool SHR1Bit> void OpDispatchBuilder::SHROp(OpcodeArgs) {
    OrderedNode *Src;
  ...
  template<bool SHR1Bit> void OpDispatchBuilder::ASHROp(OpcodeArgs) {
    OrderedNode *Src;
    OrderedNode *Dest = LoadSource(GPRClass, Op, Op->Dest, Op->Flags);
  ...
  template<bool Is1Bit> void OpDispatchBuilder::ROROp(OpcodeArgs) {
    OrderedNode *Src;
    OrderedNode *Dest = LoadSource(GPRClass, Op, Op->Dest, Op->Flags);
  ...
  template<bool Is1Bit> void OpDispatchBuilder::ROLOp(OpcodeArgs) {
    OrderedNode *Src;
    OrderedNode *Dest = LoadSource(GPRClass, Op, Op->Dest, Op->Flags);
    ...
    auto ALUOp =
    _Ror(Size == 64 ? OpSize::i64Bit : OpSize::i32Bit, Dest, _Sub(Size == 64 ? OpSize::i64Bit : OpSize::i32Bit, _Constant(Size, std::max(32U, Size)), Src));
  ...
  void OpDispatchBuilder::RCROp(OpcodeArgs) {
    ...
    if (IsValueConstant(WrapNode(Src), &Const)) {
      ...
      if (Const > 1) Res = _Orlshl(OpSize, Res, Dest, Size + 1 - Const);
  ...
  void OpDispatchBuilder::RCLOp(OpcodeArgs) {
    ...
    if (IsValueConstant(WrapNode(Src), &Const)) {
      ...
      if (Const > 1) Res = _Orlshr(OpSize, Res, Dest, Size + 1 - Const);
  ...
  template<uint32_t SrcIndex, BTAction Action> void OpDispatchBuilder::BTOp(OpcodeArgs) {
    OrderedNode *Value;
    OrderedNode *Src{};
  ...
  void OpDispatchBuilder::DAAOp(OpcodeArgs) {
    ...
    auto Cond =
    _Or(OpSize::i64Bit, AF, _Select(FEXCore::IR::COND_UGT, _And(OpSize::i64Bit, AL, _Constant(0xF)), _Constant(9), _Constant(1), _Constant(0)));
  ...
  template<OpDispatchBuilder::Segment Seg> void OpDispatchBuilder::ReadSegmentReg(OpcodeArgs) {
    // 64-bit only
  ...
  template<OpDispatchBuilder::Segment Seg> void OpDispatchBuilder::WriteSegmentReg(OpcodeArgs) {
  ...
  OrderedNode *OpDispatchBuilder::LoadSource_WithOpSize(
  ...
  OrderedNode *OpDispatchBuilder::LoadSource(
  ...
  void OpDispatchBuilder::StoreResult_WithOpSize(
  FEXCore::IR::RegisterClassType Class, FEXCore::X86Tables::DecodedOp Op, FEXCore::X86Tables::DecodedOperand const &Operand,
  OrderedNode * const Src, uint8_t OpSize, int8_t Align, MemoryAccessType AccessType) {
    LOGMAN_THROW_A_FMT(
    Operand.IsGPR() || Operand.IsLiteral() || Operand.IsGPRDirect() || Operand.IsGPRIndirect() || Operand.IsRIPRelative() || Operand.IsSIB(),
  ...
  void OpDispatchBuilder::StoreResult(
  FEXCore::IR::RegisterClassType Class, FEXCore::X86Tables::DecodedOp Op, FEXCore::X86Tables::DecodedOperand const &Operand,
  OrderedNode * const Src, int8_t Align, MemoryAccessType AccessType) {
    StoreResult_WithOpSize(Class, Op, Operand, Src, GetDstSize(Op), Align, AccessType);
  }

  void OpDispatchBuilder::StoreResult(
  FEXCore::IR::RegisterClassType Class, FEXCore::X86Tables::DecodedOp Op, OrderedNode * const Src, int8_t Align, MemoryAccessType AccessType) {
    StoreResult(Class, Op, Op->Dest, Src, Align, AccessType);
  }

  OpDispatchBuilder::OpDispatchBuilder(FEXCore::Context::ContextImpl *ctx) : IREmitter{ctx->OpDispatcherAllocator}, CTX{ctx} {
    ResetWorkingList();
    InstallHostSpecificOpcodeHandlers();
  }
  OpDispatchBuilder::OpDispatchBuilder(FEXCore::Utils::IntrusivePooledAllocator &Allocator) : IREmitter{Allocator}, CTX{nullptr} {}
  ...
  template<uint32_t SrcIndex> void OpDispatchBuilder::MOVGPROp(OpcodeArgs) {
  ...
  void OpDispatchBuilder::ALUOpImpl(OpcodeArgs, FEXCore::IR::IROps ALUIROp, FEXCore::IR::IROps AtomicFetchOp) {
    ...
    // Logical ops can tolerate garbage in the upper bits, so don't mask.
    bool AllowUpperGarbage = ALUIROp == FEXCore::IR::IROps::OP_AND || ALUIROp == FEXCore::IR::IROps::OP_XOR || ALUIROp == FEXCore::IR::IROps::OP_OR;

    // X86 basic ALU ops just do the operation between the destination and a single source
    OrderedNode *Src = LoadSource(GPRClass, Op, Op->Src[0], Op->Flags, {.AllowUpperGarbage = AllowUpperGarbage || Size >= 4});
  ...
  template<FEXCore::IR::IROps ALUIROp, FEXCore::IR::IROps AtomicFetchOp> void OpDispatchBuilder::ALUOp(OpcodeArgs) {
  ...
  template<bool Reseed> void OpDispatchBuilder::RDRANDOp(OpcodeArgs) {
