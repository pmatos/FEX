...
namespace FEXCore::ARMEmitter {
  ...
  [[nodiscard]] constexpr size_t RegSizeInBits(Size size) { return size_t{32} << FEXCore::ToUnderlying(size); }
  ...
  [[nodiscard]] constexpr size_t SubRegSizeInBits(SubRegSize size) { return size_t{8} << FEXCore::ToUnderlying(size); }
  ...
  [[nodiscard]] constexpr size_t ScalarRegSizeInBits(ScalarRegSize size) { return size_t{8} << FEXCore::ToUnderlying(size); }
  ...
  [[nodiscard]] constexpr VectorRegSizePair ToVectorSizePair(SubRegSize size) { return VectorRegSizePair{.Vector = size}; }
  [[nodiscard]] constexpr VectorRegSizePair ToVectorSizePair(ScalarRegSize size) { return VectorRegSizePair{.Scalar = size}; }
  ...
  public:
    ...
    [[nodiscard]] bool IsScalarPlusScalar() const { return MemType == Type::ScalarPlusScalar; }
    [[nodiscard]] bool IsScalarPlusImm() const { return MemType == Type::ScalarPlusImm; }
    [[nodiscard]] bool IsScalarPlusVector() const { return MemType == Type::ScalarPlusVector; }
    [[nodiscard]] bool IsVectorPlusImm() const { return MemType == Type::VectorPlusImm; }
  ...
  template<uint32_t op0, uint32_t op1, uint32_t CRn, uint32_t CRm, uint32_t op2> constexpr uint32_t GenSystemReg() {
  ...
  template<uint32_t op1, uint32_t CRm, uint32_t op2> constexpr uint32_t GenDCReg() { return op1 << 16 | CRm << 8 | op2 << 5; };
  ...
  template<uint32_t CRm, uint32_t op2> constexpr uint32_t GenHintBarrierReg() { return CRm << 8 | op2 << 5; }
  ...
  static inline void AddLocationToLabel(SingleUseForwardLabel *Label, SingleUseForwardLabel &&Location) {
    LOGMAN_THROW_A_FMT(
    Label->Type == SingleUseForwardLabel::InstType::UNKNOWN,
    "Trying to bind a SingleUseForwardLabel to multiple locations. Use ForwardLabel instead.");
  ...
  template<typename T> concept IsXOrWRegister = std::is_same_v<T, XRegister> || std::is_same_v<T, WRegister>;
  ...
  //
  template<typename T, typename... Args> constexpr bool AreVectorsSequential(T first, const Args &...args) {
  ...
  public:
    ...
    Emitter(uint8_t *Base, uint64_t BaseSize) : Buffer(Base, BaseSize) {}
    ...
    template<bool WarnAboutEmpty = false> void Bind(ForwardLabel *Label) {
...
#include "Interface/Core/ArchHelpers/CodeEmitter/SVEOps.inl"
  ...
  private:
    template<typename T> uint32_t Encode_ra(T Reg) const { return Reg.Idx() << 10; }
    uint32_t Encode_ra(uint32_t Reg) const { return Reg << 10; }
    template<typename T> uint32_t Encode_rt2(T Reg) const { return Reg.Idx() << 10; }
    template<> uint32_t Encode_rt2(uint32_t Reg) const { return Reg << 10; }
    template<typename T> uint32_t Encode_rm(T Reg) const { return Reg.Idx() << 16; }
    uint32_t Encode_rm(uint32_t Reg) const { return Reg << 16; }
    template<typename T> uint32_t Encode_rs(T Reg) const { return Reg.Idx() << 16; }
    uint32_t Encode_rs(uint32_t Reg) const { return Reg << 16; }
    template<typename T> uint32_t Encode_rn(T Reg) const { return Reg.Idx() << 5; }
    uint32_t Encode_rn(uint32_t Reg) const { return Reg << 5; }
    template<typename T> uint32_t Encode_rd(T Reg) const { return Reg.Idx(); }
    uint32_t Encode_rd(uint32_t Reg) const { return Reg; }
    template<typename T> uint32_t Encode_rt(T Reg) const { return Reg.Idx(); }
    template<> uint32_t Encode_rt(Prefetch Reg) const { return FEXCore::ToUnderlying(Reg); }
    uint32_t Encode_rt(uint32_t Reg) const { return Reg; }
    template<typename T> uint32_t Encode_pd(T Reg) const { return FEXCore::ToUnderlying(Reg); }
  };
}
