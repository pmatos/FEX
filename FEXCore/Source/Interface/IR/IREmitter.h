...
namespace FEXCore::IR {
  ...
  public:
    IREmitter(FEXCore::Utils::IntrusivePooledAllocator &ThreadAllocator) : DualListData{ThreadAllocator, 8 * 1024 * 1024} {
      ReownOrClaimBuffer();
      ResetWorkingList();
    }
    ...
    void ReownOrClaimBuffer() { DualListData.ReownOrClaimBuffer(); }

    void DelayedDisownBuffer() { DualListData.DelayedDisownBuffer(); }
...
#include <FEXCore/IR/IRDefines.inc>
    ...
    IRPair<IROp_Jump> _Jump() { return _Jump(InvalidNode); }
    ...
    IRPair<IROp_Select> _Select(uint8_t Cond, OrderedNode *ssa0, OrderedNode *ssa1, OrderedNode *ssa2, OrderedNode *ssa3, uint8_t CompareSize = 0) {
      if (CompareSize == 0) CompareSize = std::max<uint8_t>(4, std::max<uint8_t>(GetOpSize(ssa0), GetOpSize(ssa1)));

      return _Select(
      IR::SizeToOpSize(std::max<uint8_t>(4, std::max<uint8_t>(GetOpSize(ssa2), GetOpSize(ssa3)))), IR::SizeToOpSize(CompareSize),
    ...
    OrderedNode *Invalid() { return InvalidNode; }

    void SetJumpTarget(IR::IROp_Jump *Op, OrderedNode *Target) {
      LOGMAN_THROW_A_FMT(
      Target->Op(DualListData.DataBegin())->Op == OP_CODEBLOCK, "Tried setting Jump target to %{} {}",
    ...
    void SetTrueJumpTarget(IR::IROp_CondJump *Op, OrderedNode *Target) {
      LOGMAN_THROW_A_FMT(
      Target->Op(DualListData.DataBegin())->Op == OP_CODEBLOCK, "Tried setting CondJump target to %{} {}",
      Target->Wrapped(DualListData.ListBegin()).ID(), IR::GetName(Target->Op(DualListData.DataBegin())->Op));
    ...
    OrderedNode *UnwrapNode(OrderedNodeWrapper ssa) { return ssa.GetNode(DualListData.ListBegin()); }

    OrderedNodeWrapper WrapNode(OrderedNode *node) { return node->Wrapped(DualListData.ListBegin()); }
    ...
    void CopyData(IREmitter const &rhs) {
      LOGMAN_THROW_A_FMT(
      rhs.DualListData.DataBackingSize() <= DualListData.DataBackingSize(), "Trying to take ownership of data that is too large");
      LOGMAN_THROW_A_FMT(
      rhs.DualListData.ListBackingSize() <= DualListData.ListBackingSize(), "Trying to take ownership of data that is too large");
    ...
    void SetWriteCursor(OrderedNode *Node) { CurrentWriteCursor = Node; }

    OrderedNode *GetWriteCursor() { return CurrentWriteCursor; }

    OrderedNode *GetCurrentBlock() { return CurrentCodeBlock; }
