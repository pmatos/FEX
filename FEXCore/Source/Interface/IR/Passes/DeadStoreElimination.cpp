...
namespace FEXCore::IR {
  ...
  private:
    ...
    bool IsTrackedWriteFPR(uint32_t Offset, uint8_t Size) const {
      if (Size != 16 && Size != 8 && Size != 4) return false;
    ...
    uint64_t FPRBit(uint32_t Offset, uint32_t Size) const {
      ...
      if (!IsTrackedWriteFPR(Offset, Size)) return 7UL << (bitn);
  ...
  bool IsFullGPR(uint32_t Offset, uint8_t Size) {
    ...
    if (Offset < 8 || Offset >= (17 * 8)) return false;
    ...
    // Remove the dead stores
    {
      for (auto [BlockNode, BlockIROp] : CurrentIR.GetBlocks()) {
        for (auto [CodeNode, IROp] : CurrentIR.GetCode(BlockNode)) {
          ...
          auto RemoveDeadRegisterStore =
          [this](FEXCore::IR::IREmitter* IREmit, FEXCore::IR::OrderedNode* CodeNode, Info& BlockInfo, uint32_t Offset, uint8_t Size) -> bool {
            bool Changed{};
            //// GPRs ////
