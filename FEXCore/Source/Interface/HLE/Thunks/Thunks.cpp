...
namespace FEXCore {
  ...
  struct ThunkHandler_impl final : public ThunkHandler {
    ...
    static void CallCallback(void* callback, void* arg0, void* arg1) {
      ...
      if (CTX->Config.Is64BitMode) {
        ...
        thread_local void* local_args = mmap(0, 128, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    ...
    static void LoadLib(void* ArgsV) {
      ...
      auto SOName = (CTX->Config.Is64BitMode() ? CTX->Config.ThunkHostLibsPath() : CTX->Config.ThunkHostLibsPath32()) + "/" + Name.data() +
      "-host.so";
  ...
  HostToGuestTrampolinePtr* MakeHostTrampolineForGuestFunction(void* HostPacker, uintptr_t GuestTarget, uintptr_t GuestUnpacker) {
    ...
    if (ThunkHandler->HostTrampolineInstanceDataAvailable < HostToGuestTrampolineSize) {
      ...
      ThunkHandler->HostTrampolineInstanceDataPtr =
      (uint8_t*)mmap(0, ThunkHandler->HostTrampolineInstanceDataAvailable, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  ...
  void FinalizeHostTrampolineForGuestFunction(HostToGuestTrampolinePtr* TrampolineAddress, void* HostPacker) {
    ...
    LOGMAN_THROW_A_FMT(
    Trampoline.CallCallback == (uintptr_t)&ThunkHandler_impl::CallCallback, "Invalid trampoline at {} passed to {}",
