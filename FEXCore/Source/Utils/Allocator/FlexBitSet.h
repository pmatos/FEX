...
namespace FEXCore {
  ...
  template<typename T> struct FlexBitSet final {
    using ElementType = T;
    ...
    bool Get(size_t Element) const { return (Memory[Element / MinimumSizeBits] & (1ULL << (Element % MinimumSizeBits))) != 0; }
    bool TestAndClear(size_t Element) {
      bool Value = Get(Element);
      Memory[Element / MinimumSizeBits] &= ~(1ULL << (Element % MinimumSizeBits));
      return Value;
    }
    void Set(size_t Element) { Memory[Element / MinimumSizeBits] |= (1ULL << (Element % MinimumSizeBits)); }
    void Clear(size_t Element) { Memory[Element / MinimumSizeBits] &= ~(1ULL << (Element % MinimumSizeBits)); }
    void MemClear(size_t Elements) { memset(Memory, 0, FEXCore::AlignUp(Elements / MinimumSizeBits, MinimumSizeBits)); }
    void MemSet(size_t Elements) { memset(Memory, 0xFF, FEXCore::AlignUp(Elements / MinimumSizeBits, MinimumSizeBits)); }
    ...
    template<bool WantUnset> BitsetScanResults BackwardScanForRange(size_t BeginningElement, size_t ElementCount, size_t MinimumElement) {
      bool FoundHole{};
    ...
    //
    // @return The scan results
    template<bool WantUnset> BitsetScanResults ForwardScanForRange(size_t BeginningElement, size_t ElementCount, size_t ElementsInSet) {
      bool FoundHole{};
    ...
    // Is only a getter
    bool operator[](size_t Element) const { return Get(Element); }

    static size_t Size(uint64_t Elements) { return FEXCore::AlignUp(Elements / MinimumSizeBits, MinimumSizeBits); }
  };
