...
namespace Alloc {
  ...
  public:
    ForwardOnlyIntrusiveArenaAllocator(void *Ptr, size_t _Size) : Begin{reinterpret_cast<uintptr_t>(Ptr)}, Size{_Size} {
      LastAllocation = sizeof(ForwardOnlyIntrusiveArenaAllocator);
    }

    ~ForwardOnlyIntrusiveArenaAllocator() = default;

    template<class U, class... Args> U *new_construct(Args &&...args) {
      void *Ptr = do_allocate(sizeof(U), std::alignment_of<U>::value);
      return new (Ptr) U(args...);
    }

    template<class U, class... Args> U *new_construct(U *Class, Args &&...args) {
      void *Ptr = do_allocate(sizeof(U), std::alignment_of<U>::value);
      return new (Ptr) U(args...);
    }
  ...
  public:
    IntrusiveArenaAllocator(void *Ptr, size_t _Size) : Begin{reinterpret_cast<uintptr_t>(Ptr)}, Size{_Size} {
      uint64_t NumberOfPages = _Size / FHU::FEX_PAGE_SIZE;
      uint64_t UsedBits = FEXCore::AlignUp(sizeof(IntrusiveArenaAllocator) + Size / FHU::FEX_PAGE_SIZE / 8, FHU::FEX_PAGE_SIZE);
