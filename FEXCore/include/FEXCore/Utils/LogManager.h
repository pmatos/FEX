...
// The assume version should be preferred unless what is being checked has side effects.
    ...
    template<typename... Args> static inline void AFmt(bool Value, const char* fmt, const Args&... args) {
      if (MSG_LEVEL < ASSERT || Value) {
        return;
      }
      MFmt(fmt, fmt::make_format_args(args...));
    }
    template<typename... Args> static inline void AAFmt(bool Value, const char* fmt, const Args&... args) {
      if (MSG_LEVEL < ASSERT || Value) {
        return;
      }
      MFmt(fmt, fmt::make_format_args(args...));
...
#define LOGMAN_THROW_AA_FMT(pred, ...) \
  ...
  namespace Msg {
    ...
    template<typename... Args> static inline void MFmt(DebugLevels level, const char* fmt, const Args&... args) {
      MFmtImpl(level, fmt, fmt::make_format_args(args...));
    }

    template<typename... Args> static inline void EFmt(const char* fmt, const Args&... args) {
      if (MSG_LEVEL < ERROR) {
        return;
      }
    ...
    template<typename... Args> static inline void DFmt(const char* fmt, const Args&... args) {
      if (MSG_LEVEL < DEBUG) {
        return;
      }
    ...
    template<typename... Args> static inline void IFmt(const char* fmt, const Args&... args) {
      if (MSG_LEVEL < INFO) {
        return;
      }
    ...
    template<typename... Args> static inline void OutFmt(const char* fmt, const Args&... args) {
      MFmtImpl(STDOUT, fmt, fmt::make_format_args(args...));
    }

    template<typename... Args> static inline void ErrFmt(const char* fmt, const Args&... args) {
      MFmtImpl(STDERR, fmt, fmt::make_format_args(args...));
    }

#if defined(ASSERTIONS_ENABLED) && ASSERTIONS_ENABLED
    template<typename... Args> static inline void AFmt(const char* fmt, const Args&... args) {
      if (MSG_LEVEL < ASSERT) {
        return;
      }
...
#else
    template<typename... Args> static inline void AFmt(const char*, const Args&...) {}
#define LOGMAN_MSG_A_FMT(...) \
  do { \
  } while (0)
#endif
