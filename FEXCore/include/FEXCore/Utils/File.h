...
namespace FEXCore::File {
  ...
  public:
    ...
    ssize_t Write(std::string_view const Data) { return Write(Data.data(), Data.size()); }
  ...
  protected:
    ...
    File(FileHandleType Handle, bool ShouldClose) : ShouldClose{ShouldClose}, IsValidHandle{true}, Handle{Handle} {}
  private:
    bool ShouldClose{};
    bool IsValidHandle{};
    ...
    static uint32_t TranslateModes(FileModes Modes) {
      uint32_t Mode{};
      if ((Modes & FileModes::READ) == FileModes::READ) Mode |= O_RDONLY;
      if ((Modes & FileModes::WRITE) == FileModes::WRITE) Mode |= O_WRONLY;
      if ((Modes & FileModes::CREATE) == FileModes::CREATE) Mode |= O_CREAT;
      if ((Modes & FileModes::TRUNCATE) == FileModes::TRUNCATE) Mode |= O_TRUNC;
    ...
    static Disposition TranslateModes(FileModes Modes) {
      Disposition Disp{};
      if ((Modes & FileModes::READ) == FileModes::READ) Disp.Access |= GENERIC_READ;
      if ((Modes & FileModes::WRITE) == FileModes::WRITE) Disp.Access |= GENERIC_WRITE;
      if ((Modes & FileModes::CREATE) == FileModes::CREATE)
        Disp.CreationFlag = CREATE_ALWAYS;
      else
        Disp.CreationFlag = OPEN_ALWAYS;

      if ((Modes & FileModes::TRUNCATE) == FileModes::TRUNCATE) Disp.TruncateOnExist = true;
