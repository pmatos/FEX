...
namespace FEXCore {
  ...
  public:
    ForkableUniqueMutex() : Mutex(PTHREAD_MUTEX_INITIALIZER) {}
    ...
    void StealAndDropActiveLocks() { Mutex = PTHREAD_MUTEX_INITIALIZER; }
  private:
  ...
  public:
    ForkableSharedMutex() : Mutex(PTHREAD_RWLOCK_INITIALIZER) {}
    ...
    // Initialize the internal pthread object to its default initializer state.
    // Should only ever be used in the child process when a Linux fork() has occured.
    void StealAndDropActiveLocks() { Mutex = PTHREAD_RWLOCK_INITIALIZER; }
  private:
  ...
  public:
    void StealAndDropActiveLocks() { LogMan::Msg::AFmt("{} is unsupported on WIN32 builds!", __func__); }
  };
  ...
  public:
    ...
    DeferredSignalRefCountGuard(DeferredSignalRefCountGuard&& rhs) : Thread(rhs.Thread) { rhs.Thread = nullptr; }
  ...
  public:
    ...
    ScopedSignalMasker(ScopedSignalMasker&& rhs) : OriginalMask(rhs.OriginalMask) { rhs.OriginalMask.reset(); }
  ...
  [[nodiscard]] static auto GuardSignalDeferringSectionWithFallback(MutexType& mutex, FEXCore::Core::InternalThreadState* Thread, uint64_t Mask = ~0ULL) {
    ...
    Thread
    ? ExtraGuard{DeferredSignalRefCountGuard{Thread}}
#ifndef _WIN32
