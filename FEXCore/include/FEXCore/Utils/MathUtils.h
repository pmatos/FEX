...
namespace FEXCore {
  [[nodiscard]] constexpr uint64_t AlignUp(uint64_t value, uint64_t size) { return value + (size - value % size) % size; }

  [[nodiscard]] constexpr uint64_t AlignDown(uint64_t value, uint64_t size) { return value - value % size; }

  // Returns the ilog2 of a power-of-2 integer.
  // Asserts in the case that the passed in integer is not a power-of-2.
  template<typename T> requires(std::is_unsigned_v<T>) [[nodiscard]] constexpr T ilog2(T Value) {
  ...
  requires(std::is_unsigned_v<T>&& std::is_unsigned_v<TT>) [[nodiscard]] constexpr T DividePow2(T Dividend, TT Divisor) {
