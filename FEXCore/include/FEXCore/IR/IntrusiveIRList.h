 ...
 */
  ...
  protected:
    DualIntrusiveAllocator(size_t Size) : MemorySize{Size} {}
  ...
  public:
    DualIntrusiveAllocatorMalloc(size_t Size) : DualIntrusiveAllocator{Size} {
      Data = reinterpret_cast<uintptr_t>(FEXCore::Allocator::malloc(Size * 2));
      List = reinterpret_cast<uintptr_t>(Data + Size);
    }

    ~DualIntrusiveAllocatorMalloc() { FEXCore::Allocator::free(reinterpret_cast<void *>(Data)); }
  };
  ...
  public:
    ...
    ~DualIntrusiveAllocatorThreadPool() { PoolObject.UnclaimBuffer(); }
    ...
    void DelayedDisownBuffer() { PoolObject.DelayedDisownBuffer(); }
  ...
  public:
    ...
    [[nodiscard]] IRListView *CreateCopy() { return new IRListView(this, true); }
    ...
    [[nodiscard]] bool IsCopy() const { return (Flags & FLAG_IsCopy) != 0; }
    ...
    [[nodiscard]] bool IsShared() const { return (Flags & FLAG_Shared) != 0; }
    ...
    [[nodiscard]] NodeID GetID(const OrderedNode *Node) const { return Node->Wrapped(GetListData()).ID(); }
    ...
    [[nodiscard]] IROp_IRHeader *GetHeader() const { return GetOp<IROp_IRHeader>(GetHeaderNode()); }

    template<typename T> [[nodiscard]] T *GetOp(OrderedNode *Node) const {
      ...
      if constexpr (!std::is_same<T, IROp_Header>::value) {
        LOGMAN_THROW_A_FMT(
        Op->OPCODE == Op->Header.Op, "Expected Node to be '{}'. Found '{}' instead", GetName(Op->OPCODE), GetName(Op->Header.Op));
      }

      return Op;
    }

    template<typename T> [[nodiscard]] T *GetOp(OrderedNodeWrapper Wrapper) const {
      auto Node = Wrapper.GetNode(GetListData());
      return GetOp<T>(Node);
    }

    [[nodiscard]] OrderedNode *GetNode(OrderedNodeWrapper Wrapper) const { return Wrapper.GetNode(GetListData()); }

    ///< Gets an OrderedNode from the IRListView as an OrderedNodeWrapper.
    [[nodiscard]] OrderedNodeWrapper WrapNode(OrderedNode *Node) const { return Node->Wrapped(GetListData()); }
  ...
  private:
    struct BlockRange {
      ...
      [[nodiscard]] iterator end() const noexcept { return iterator(View->GetListData(), View->GetData()); }
    };
  ...
  public:
    ...
    [[nodiscard]] BlockRange GetBlocks() const { return BlockRange(this); }

    [[nodiscard]] CodeRange GetCode(const OrderedNode *block) const { return CodeRange(this, block->Wrapped(GetListData())); }

    [[nodiscard]] AllCodeRange GetAllCode() const { return AllCodeRange(this); }
   ...
   */
    [[nodiscard]] iterator at(OrderedNodeWrapper Wrapped) const noexcept { return iterator(GetListData(), GetData(), Wrapped); }
    ...
    [[nodiscard]] uintptr_t GetData() const { return reinterpret_cast<uintptr_t>(IRDataInternal ? IRDataInternal : InlineData); }
