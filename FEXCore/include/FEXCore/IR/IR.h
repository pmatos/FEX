 ...
 */
  struct NodeID final {
    ...
    constexpr void Invalidate() noexcept { Value = 0; }

    [[nodiscard]] friend constexpr bool operator==(NodeID, NodeID) noexcept = default;

    [[nodiscard]] friend constexpr bool operator<(NodeID lhs, NodeID rhs) noexcept { return lhs.Value < rhs.Value; }
    [[nodiscard]] friend constexpr bool operator>(NodeID lhs, NodeID rhs) noexcept { return operator<(rhs, lhs); }
    [[nodiscard]] friend constexpr bool operator<=(NodeID lhs, NodeID rhs) noexcept { return !operator>(lhs, rhs); }
    [[nodiscard]] friend constexpr bool operator>=(NodeID lhs, NodeID rhs) noexcept { return !operator<(lhs, rhs); }
 ...
 */
  template<typename Type> struct NodeWrapperBase final {
    ...
    [[nodiscard]] static void *UnwrapNode(uintptr_t Base, NodeWrapperBase Node) { return Node.GetNode(Base); }

    [[nodiscard]] NodeID ID() const;

    [[nodiscard]] bool IsInvalid() const { return NodeOffset == 0; }

    [[nodiscard]] Type *GetNode(uintptr_t Base) { return reinterpret_cast<Type *>(Base + NodeOffset); }
    [[nodiscard]] const Type *GetNode(uintptr_t Base) const { return reinterpret_cast<const Type *>(Base + NodeOffset); }
 ...
 */
  ...
  public:
    ...
    [[nodiscard]] IROp_Header const *Op(uintptr_t Base) const { return Header.Value.GetNode(Base); }
    [[nodiscard]] IROp_Header *Op(uintptr_t Base) { return Header.Value.GetNode(Base); }
  ...
  struct RegisterClassType final {
    ...
    value_type Val;
    [[nodiscard]] constexpr operator value_type() const { return Val; }
  ...
  struct CondClassType final {
    uint8_t Val;
    [[nodiscard]] constexpr operator uint8_t() const { return Val; }
  ...
  struct TypeDefinition final {
    ...
    [[nodiscard]] constexpr operator uint16_t() const { return Val; }
    ...
    [[nodiscard]] constexpr uint8_t Bytes() const { return Val >> 8; }

    [[nodiscard]] constexpr uint8_t Elements() const { return Val & 255; }
  ...
  struct SHA256Sum final {
    uint8_t data[32];
    [[nodiscard]] bool operator<(SHA256Sum const &rhs) const { return memcmp(data, rhs.data, sizeof(data)) < 0; }
 ...
 */
  ...
  public:
    ...
    [[nodiscard]] bool operator==(const NodeIterator &rhs) const { return Node.NodeOffset == rhs.Node.NodeOffset; }

    [[nodiscard]] bool operator!=(const NodeIterator &rhs) const { return !operator==(rhs); }
    ...
    [[nodiscard]] NodeID ID() const { return Node.ID(); }

    [[nodiscard]] static NodeIterator Invalid() { return NodeIterator(0, 0); }
 ...
 */
  ...
  public:
    ...
    [[nodiscard]] static AllNodesIterator Invalid() { return AllNodesIterator(0, 0); }
  };

  class IRListView;
  class IREmitter;

  template<typename Type> inline NodeID NodeWrapperBase<Type>::ID() const { return NodeID(NodeOffset / sizeof(IR::OrderedNode)); }
...
template<> struct std::hash<FEXCore::IR::NodeID> {
  size_t operator()(const FEXCore::IR::NodeID &ID) const noexcept { return std::hash<FEXCore::IR::NodeID::value_type>{}(ID.Value); }
};

template<> struct fmt::formatter<FEXCore::IR::NodeID> : fmt::formatter<FEXCore::IR::NodeID::value_type> {
  ...
  template<typename FormatContext> auto format(const FEXCore::IR::NodeID &ID, FormatContext &ctx) const {
    return Base::format(ID.Value, ctx);
  }
};

template<> struct fmt::formatter<FEXCore::IR::RegisterClassType> : fmt::formatter<FEXCore::IR::RegisterClassType::value_type> {
  using Base = fmt::formatter<FEXCore::IR::RegisterClassType::value_type>;

  template<typename FormatContext> auto format(const FEXCore::IR::RegisterClassType &Class, FormatContext &ctx) const {
    return Base::format(Class.Val, ctx);
  }
};

template<> struct fmt::formatter<FEXCore::IR::FenceType> : fmt::formatter<FEXCore::IR::FenceType::value_type> {
  using Base = fmt::formatter<FEXCore::IR::FenceType::value_type>;

  template<typename FormatContext> auto format(const FEXCore::IR::FenceType &Fence, FormatContext &ctx) const {
    return Base::format(Fence.Val, ctx);
  }
};

template<> struct fmt::formatter<FEXCore::IR::OpSize> : fmt::formatter<std::underlying_type_t<FEXCore::IR::OpSize>> {
  using Base = fmt::formatter<std::underlying_type_t<FEXCore::IR::OpSize>>;

  template<typename FormatContext> auto format(const FEXCore::IR::OpSize &OpSize, FormatContext &ctx) const {
