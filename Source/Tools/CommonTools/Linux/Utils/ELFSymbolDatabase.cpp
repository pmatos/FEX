...
namespace ELFLoader {
  ...
  ELFSymbolDatabase::ELFSymbolDatabase(::ELFLoader::ELFContainer *file) : File{file} {
  ...
  void ELFSymbolDatabase::FillInitializationOrder() {
    ...
    while (true) {
      if (InitializationOrder.size() == DynamicELFInfo.size()) break;

      for (auto &ELF : DynamicELFInfo) {
        // If this ELF is already in the list then skip it
        if (AlreadyInList.find(ELF->Name) != AlreadyInList.end()) continue;
  ...
  void ELFSymbolDatabase::HandleRelocations() {
    auto SymbolGetter = [this](char const *SymbolName, uint8_t Table) -> ELFLoader::ELFSymbol * {
      ...
      if (Sym == TablePtr.end()) return nullptr;
      return Sym->second;
    };
  ...
  ELFSymbol const *ELFSymbolDatabase::GetSymbolInRange(RangeType Address) {
    ...
    if (Sym != SymbolMapByAddress.begin()) --Sym;
    if (Sym == SymbolMapByAddress.end()) return nullptr;

    if ((Sym->second->Address + Sym->second->Size) < Address.first) return nullptr;

    if (Sym->second->Address > Address.first) return nullptr;
