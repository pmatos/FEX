...
namespace ELFLoader {
  ...
  ELFSymbol const *ELFContainer::GetSymbol(char const *Name) {
    ...
    if (Sym == SymbolMap.end()) return nullptr;
    return Sym->second;
  }
  ...
  void ELFContainer::CalculateSymbols() {
    ...
    if (Mode == MODE_32BIT) {
      ...
      for (uint64_t i = 0; i < NumSymTabSymbols; ++i) {
        ...
        Elf32_Sym const *Symbol = reinterpret_cast<Elf32_Sym const *>(&RawFile.at(offset));
      ...
      for (uint64_t i = 0; i < NumSymTabSymbols; ++i) {
        uint64_t offset = SymTabHeader->sh_offset + i * SymTabHeader->sh_entsize;
        Elf64_Sym const *Symbol = reinterpret_cast<Elf64_Sym const *>(&RawFile.at(offset));
