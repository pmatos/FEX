...
namespace WebFileFetcher {
  ...
  std::optional<std::vector<FileTargets>> GetRootFSLinks() {
    ...
    for (json_t const *RootItem = json_getChild(RootList); RootItem != nullptr; RootItem = json_getSibling(RootItem)) {

      FileTargets Target;
      Target.DistroName = json_getName(RootItem);

      for (json_t const *DataItem = json_getChild(RootItem); DataItem != nullptr; DataItem = json_getSibling(DataItem)) {
