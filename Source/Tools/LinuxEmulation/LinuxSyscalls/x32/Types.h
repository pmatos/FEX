...
namespace FEX::HLE::x32 {
  ...
  template<typename T> class compat_ptr {
  public:
    compat_ptr() = delete;
    compat_ptr(uint32_t In) : Ptr{In} {}
    compat_ptr(T *In) : Ptr{static_cast<uint32_t>(reinterpret_cast<uintptr_t>(In))} {}

    template<typename T2 = T, typename = std::enable_if<!std::is_same<T2, void>::value, T2>> T2 &operator*() const { return *Interpret(); }

    T *operator->() { return Interpret(); }

    // In the case of non-void type, we can index the pointer
    template<typename T2 = T, typename = std::enable_if<!std::is_same<T2, void>::value, T2>> T2 &operator[](size_t idx) const {
      return *reinterpret_cast<T2 *>(Ptr + sizeof(T2) * idx);
    }

    // In the case of void type, we need to trivially convert
    template<typename T2 = T, typename = std::enable_if<std::is_same<T2, void>::value, T2>> operator T2 *() const {
      return reinterpret_cast<T2 *>(Ptr);
    }

    operator T *() const { return Interpret(); }
    ...
    uint32_t Ptr;
  private:
    T *Interpret() const { return reinterpret_cast<T *>(Ptr); }
  };
 ...
 * @{ */
  ...
  struct FEX_ANNOTATE("alias-x86_32-itimerval") FEX_ANNOTATE("fex-match") itimerval32 {
    ...
    itimerval32(struct itimerval spec) : it_interval{spec.it_interval}, it_value{spec.it_value} {}
  };
  /**  @} */
 ...
 * @{ */
  ...
  struct FEX_ANNOTATE("alias-x86_32-stack_t") FEX_ANNOTATE("fex-match") stack_t32 {
    ...
    stack_t32(stack_t ss) : ss_sp{ss.ss_sp} {
...
#undef COPY
  ...
  struct FEX_ANNOTATE("alias-x86_32-rusage") FEX_ANNOTATE("fex-match") rusage_32 {
    ...
    rusage_32() = delete;
    rusage_32(struct rusage usage) : ru_utime{usage.ru_utime}, ru_stime{usage.ru_stime} {
      // These only truncate
  ...
  struct FEX_ANNOTATE("alias-x86_32-itimerspec") FEX_ANNOTATE("fex-match") old_itimerspec32 {
    ...
    old_itimerspec32(struct itimerspec val) : it_interval{val.it_interval}, it_value{val.it_value} {}
  };
  ...
  template<bool Signed> struct FEX_ANNOTATE("alias-x86_32-rlimit") FEX_ANNOTATE("fex-match") rlimit32 {
    uint32_t rlim_cur;
    uint32_t rlim_max;
    rlimit32() = delete;
  ...
  struct FEX_ANNOTATE("alias-x86_32-timex") FEX_ANNOTATE("fex-match") timex32 {
    ...
    timex32(struct timex val) : time{val.time} {
      modes = val.modes;
      offset = val.offset;
      freq = val.freq;
  ...
  union FEX_ANNOTATE("alias-x86_32-sigval") FEX_ANNOTATE("fex-match") sigval32 {
    ...
    sigval32(sigval val) { sival_ptr = val.sival_ptr; }
  };
...
#define sigev_notify_thread_id _sigev_un._tid
    ...
    sigevent32(sigevent val) : sigev_value{val.sigev_value} {
  ...
  struct FEX_PACKED FEX_ANNOTATE("alias-x86_32-epoll_event") FEX_ANNOTATE("fex-match") epoll_event32 {
    ...
    epoll_event32(struct epoll_event event) : data{event.data.u64} { events = event.events; }
  };
  ...
  struct shmid_ds_32 {
    ...
    shmid_ds_32(struct shmid64_ds buf) : shm_perm{buf.shm_perm} {
  ...
  struct shmid_ds_64 {
    ...
    shmid_ds_64(struct shmid64_ds buf) : shm_perm{buf.shm_perm} {
      shm_segsz = buf.shm_segsz;
      shm_atime = buf.shm_atime;
  ...
  struct semid_ds_32 {
    ...
    semid_ds_32(struct semid64_ds buf) : sem_perm{buf.sem_perm} {
  ...
  struct msqid_ds_32 {
    ...
    msqid_ds_32(struct msqid64_ds buf) : msg_perm{buf.msg_perm} {
      // msg_first and msg_last are unused and untouched
  ...
  struct msqid_ds_64 {
    ...
    msqid_ds_64(struct msqid64_ds buf) : msg_perm{buf.msg_perm} {
      msg_stime = buf.msg_stime;
