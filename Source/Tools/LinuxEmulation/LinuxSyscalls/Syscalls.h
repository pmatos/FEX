...
namespace FEX::HLE {
  ...
  public:
    ...
    void IncrementIdleRefCount() { ++IdleWaitRefCount; }
    ...
    void InvalidateGuestCodeRange(
    FEXCore::Core::InternalThreadState *CallingThread, uint64_t Start, uint64_t Length, FEXCore::Context::CodeRangeInvalidationFn callback) {
      std::lock_guard lk(ThreadCreationMutex);
    ...
    fextl::vector<FEXCore::Core::InternalThreadState *> const *GetThreads() const { return &Threads; }
  ...
  public:
    ...
    SyscallFunctionDefinition const *GetDefinition(uint64_t Syscall) { return &Definitions.at(Syscall); }
    ...
    virtual void RegisterSyscall_32(
    int SyscallNumber, int32_t HostSyscallNumber, FEXCore::IR::SyscallFlags Flags,
#ifdef DEBUG_STRACE
    ...
    virtual void RegisterSyscall_64(
    int SyscallNumber, int32_t HostSyscallNumber, FEXCore::IR::SyscallFlags Flags,
#ifdef DEBUG_STRACE
    const fextl::string &TraceFormatString,
#endif
    ...
    static uint32_t KernelVersion(uint32_t Major, uint32_t Minor = 0, uint32_t Patch = 0) { return (Major << 24) | (Minor << 16) | Patch; }
    ...
    void SleepThread(FEXCore::Context::Context *CTX, FEXCore::Core::CpuStateFrame *Frame) override { TM.SleepThread(CTX, Frame); }
  ...
  private:
    ...
    struct MRID {
      ...
      bool operator<(const MRID &other) const { return std::tie(dev, id) < std::tie(other.dev, other.id); }
    };
    ...
    struct VMATracking {
      ...
      void SetUnsafe(
      FEXCore::Context::Context *Ctx, MappedResource *MappedResource, uintptr_t Base, uintptr_t Offset, uintptr_t Length, VMAFlags Flags,
      VMAProt Prot);
...
#define SYSCALL_ERRNO_NULL() \
  ...
  template<typename T> struct ArgToFmtString;

#define ARG_TO_STR(tpy, str) \
  template<> struct FEX::HLE::ArgToFmtString<tpy> { \
  ...
  // Pointers
  template<typename T> struct ArgToFmtString<T *> {
  ...
  template<typename... Args> fextl::string CollectArgsFmtString() {
 ...
 */
  ...
  [[maybe_unused]] static bool HasSyscallError(uint64_t Result) {
  ...
  [[maybe_unused]] static bool HasSyscallError(const void *Result) { return HasSyscallError(reinterpret_cast<uintptr_t>(Result)); }

  template<bool IncrementOffset, typename T> uint64_t GetDentsEmulation(int fd, T *dirp, uint32_t count);
