...
namespace FEX::HLE {
  ...
  template<bool IncrementOffset, typename T> uint64_t GetDentsEmulation(int fd, T *dirp, uint32_t count) {
    ...
    if (Result != -1) {
      ...
      while (TmpOffset < Result) {
        ...
        size_t NewRecLen =
        FEXCore::AlignUp(Tmp->d_reclen - (sizeof(std::remove_reference<decltype(*Tmp)>::type) - sizeof(*Outgoing)), alignof(decltype(Tmp->d_ino)));
  ...
  uint64_t ExecveHandler(const char *pathname, char * const *argv, char * const *envp, ExecveAtArgs Args) {
    ...
    uint64_t Result{};
    if (
    FEX::HLE::_SyscallHandler->IsInterpreterInstalled() && FEX::HLE::_SyscallHandler->IsInterpreter() &&
...
#undef FLAGPRINT
  ...
  uint64_t CloneHandler(FEXCore::Core::CpuStateFrame *Frame, FEX::HLE::clone3_args *args) {
    ...
    auto HasUnhandledFlags = [](FEX::HLE::clone3_args *args) -> bool {
      ...
      // CLONE_UNTRACED |
      CLONE_NEWCGROUP | CLONE_NEWUTS | CLONE_NEWUTS | CLONE_NEWIPC | CLONE_NEWUSER | CLONE_NEWPID | CLONE_NEWNET | CLONE_IO |
    ...
    if (Addr == nullptr) { // Just wants to get the location of the program break atm
      ...
      if (NewEnd < DataSpace) {
        ...
        if (NewSizeAligned < DataSpaceMaxSize) {
          ...
          uint64_t NewBRK{};
          NewBRK = (uint64_t)GuestMmap(
          Frame->Thread, (void *)(DataSpace + DataSpaceMaxSize), AllocateNewSize, PROT_READ | PROT_WRITE,
  ...
  uint64_t SyscallHandler::HandleSyscall(FEXCore::Core::CpuStateFrame *Frame, FEXCore::HLE::SyscallArguments *Args) {
    ...
    case 6:
      Result =
      std::invoke(Def.Ptr6, Frame, Args->Argument[1], Args->Argument[2], Args->Argument[3], Args->Argument[4], Args->Argument[5], Args->Argument[6]);
      break;
  ...
  void SyscallHandler::LockBeforeFork() { VMATracking.Mutex.lock(); }
      ...
      while (std::getline(Stream, Line)) {
        ...
        if (LineIsEmpty) {
          ...
          if (isHEX(Line[0])) {
            ...
            if (offs == Line.size()) continue;
            if (offs != 8 && offs != 16) continue;
