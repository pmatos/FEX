...
namespace FEX::HLE::x64 {
  void RegisterEpoll(FEX::HLE::SyscallHandler *Handler) {
    ...
    REGISTER_SYSCALL_IMPL_X64(
    epoll_pwait,
    [](FEXCore::Core::CpuStateFrame *Frame, int epfd, FEX::HLE::epoll_event_x86 *events, int maxevent, int timeout, const uint64_t *sigmask, size_t sigsetsize)
    -> uint64_t {
    ...
    if (Handler->IsHostKernelVersionAtLeast(5, 11, 0)) {
      ...
      epoll_pwait2,
      [](
      FEXCore::Core::CpuStateFrame *Frame, int epfd, FEX::HLE::epoll_event_x86 *events, int maxevent, timespec *timeout,
