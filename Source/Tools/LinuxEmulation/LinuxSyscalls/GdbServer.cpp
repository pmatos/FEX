...
namespace FEX {
  ...
  static std::string_view const &GetFlagName(unsigned Flag) { return FlagNames[Flag]; }
  ...
  fextl::string GdbServer::ReadPacket(std::iostream &stream) {
    ...
    while ((c = stream.get()) > 0) {
      ...
      case '$': // start of packet
        if (packet.size() != 0) LogMan::Msg::EFmt("Dropping unexpected data: \"{}\"", packet);
  ...
  GdbServer::HandledPacketType GdbServer::readReg(const fextl::string &packet) {
    ...
    if (addr >= offsetof(GDBContextDefinition, gregs[0]) && addr < offsetof(GDBContextDefinition, gregs[16])) {
      ...
      return {
      encodeHex((unsigned char *)(&state.mm[(addr - offsetof(GDBContextDefinition, mm[0])) / sizeof(X80Float)]), sizeof(X80Float)),
  ...
  GdbServer::HandledPacketType GdbServer::handleXfer(const fextl::string &packet) {
    ...
    auto encode = [&](fextl::string data) -> fextl::string {
      ...
      if (offset >= data.size()) return "E34"; // ERANGE
      if ((data.size() - offset) > length) return "m" + data.substr(offset, length);
    ...
    if (object == "features") {
      if (annex == "target.xml") return {encode(buildTargetXML()), HandledPacketType::TYPE_ACK};
  ...
  void GdbServer::SendPacketPair(const HandledPacketType &response) {
    ...
    response.TypeResponse != HandledPacketType::TYPE_ONLYNACK && response.TypeResponse != HandledPacketType::TYPE_ONLYACK &&
