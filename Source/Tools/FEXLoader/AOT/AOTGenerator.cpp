...
namespace FEX::AOT {
  void AOTGenSection(FEXCore::Context::Context *CTX, ELFCodeLoader::LoadedSection &Section) {
    ...
    if (!Section.Executable || Section.Size < 16) return;
    ...
    for (size_t Offset = 0; Offset < (Section.Size - 16); Offset++) {
      ...
      if (*pCode == 0xE8) {
        ...
        if (DestinationPtr[0] == 0 && DestinationPtr[1] == 0) continue; // add al, [rax], unlikely to be real code
        ...
        for (;;) {
          ...
          if (ExternalBranchesLocal.size() > 0) {
            ...
            for (auto Destination : ExternalBranchesLocal) {
              if (!(Destination >= Section.Base && Destination <= (Section.Base + Section.Size))) continue;
              if (Compiled.contains(Destination)) continue;
